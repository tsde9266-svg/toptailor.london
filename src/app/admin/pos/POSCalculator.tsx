...
const QUICK_ITEMS: Array<{ category: string; color: string; icon: string; items: Array<{ name: string; price: number }> }> = [
  {
    category: 'Trousers',
    color: 'from-blue-500 to-blue-700',
    icon: '👖',
    items: [
      { name: 'Trouser shortening', price: 18 },
      { name: 'Trouser shortening (same finish)', price: 25 },
      { name: 'Jeans shortening', price: 18 },
      { name: 'Jeans shortening (same finish)', price: 25 },
      { name: 'Trouser taper — 2 seam', price: 18 },
      { name: 'Trouser taper — 4 seam', price: 28 },
      { name: 'Trouser waist adjustment', price: 22 },
      { name: 'Jeans waist — same finish', price: 28 },
      { name: 'Rehem', price: 8 },
      { name: 'New zip (trousers)', price: 20 },
    ],
  },
  {
    ...
    icon: '🧥',
    items: [
      { name: 'Jacket sleeves — from cuff', price: 30 },
      { name: 'Jacket sleeves — from shoulder', price: 65 },
      { name: 'Jacket back take in — 1 seam', price: 18 },
      { name: 'Jacket back take in — 3 seam', price: 28 },
      { name: 'Jacket shorten', price: 35 },
      { name: 'Jacket shorten + zip', price: 45 },
      { name: 'Coat shortening', price: 45 },
      { name: 'Jacket relining', price: 75 },
      { name: 'Puffer — sleeves shorten', price: 45 },
    ],
  },
  {
    category: 'Dresses',
    ...
    icon: '👗',
    items: [
      { name: 'Plain dress shortening', price: 25 },
      { name: 'Dress shortening — 2 layers', price: 35 },
      { name: 'Dress take in sides', price: 28 },
      { name: 'New zip on dress', price: 28 },
      { name: 'Shorten straps', price: 18 },
    ],
  },
  {
    category: 'Custom',
    ...
    <div style={{ minHeight: '100vh', background: '#0f172a', color: 'white', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 380px', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Category tabs */}
          <div style={{ display: 'flex', gap: '0', borderBottom: '1px solid rgba(255,255,255,0.08)', overflowX: 'auto', padding: '0 16px' }}>
            {QUICK_ITEMS.map((cat, i) => (
              <button
                key={i}
                ...
                {cat.items.map((item, idx) => (
                  <button
                    ...
                    style={{
                      ...
                      borderLeftColor: cat.color.includes('blue') ? '#3b82f6' : cat.color.includes('emerald') ? '#10b981' : cat.color.includes('pink') ? '#ec4899' : '#8b5cf6',
                      color: 'white',
                      padding: '14px 16px',
                      textAlign: 'left',
                      borderRadius: '8px',
                  ...
                  >
                    ...
                    <span style={{
                      fontSize: '18px',
                      fontWeight: 700,
                      color: cat.color.includes('blue') ? '#60a5fa' : cat.color.includes('emerald') ? '#34d399' : cat.color.includes('pink') ? '#f9a8d4' : '#c4b5fd',
                    }}>
                      £{item.price}
                    </span>
                  </button>
        ...
        <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {cart.map((item, idx) => (
                  ...
                  >
                    ...
                    <span style={{ fontSize: '15px', fontWeight: 700, color: '#60a5fa', minWidth: '48px', textAlign: 'right' }}>
                      £{item.price}
                    </span>
                    <button
          ...
          <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '8px' }}>
              <button
                ...
                style={{
                  background: discountPct === 0 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.06)',
                  border: `1px solid ${discountPct === 0 ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.1)'}`,
                  color: 'white',
                  padding: '6px 12px',
                  fontSize: '13px',
                  borderRadius: '6px',
              ...
              {DISCOUNT_PRESETS.map(pct => (
                <button
                  ...
                  style={{
                    background: discountPct === pct ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.06)',
                    border: `1px solid ${discountPct === pct ? '#10b981' : 'rgba(255,255,255,0.1)'}`,
                    color: discountPct === pct ? '#6ee7b7' : 'white',
                    padding: '6px 12px',
                    fontSize: '13px',
                    borderRadius: '6px',
                    cursor: 'pointer',
                    fontWeight: discountPct === pct ? 700 : 400,
                  }}
                >
                  {pct}%
                </button>
          ...
          <div style={{ padding: '16px', background: 'rgba(0,0,0,0.3)', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '14px', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
                Total
              </span>
              <span style={{
                fontSize: '40px',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {cart.length > 0 && (
                <a
                  ...
                  style={{
                    display: 'block',
                    background: 'linear-gradient(135deg, #2A5220, #3B6D11)',
                    color: 'white',
                    textDecoration: 'none',
                    padding: '14px',
                    textAlign: 'center',
              ...
              <button
                ...
                style={{
                  ...
                  border: '1px solid rgba(255,255,255,0.15)',
                  color: cart.length === 0 ? 'rgba(255,255,255,0.2)' : 'white',
                  padding: '12px',
                  textAlign: 'center',
                  fontSize: '13px',
                  fontWeight: 600,
